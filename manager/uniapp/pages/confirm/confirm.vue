<template>
	<view style="padding: 20rpx;">
		...
		<!-- 提交按钮 -->
		<view class="box"
			style="position: fixed; bottom: 0; width: 100vw; left: 0; z-index: 10; background-color: #fff; box-shadow: 0 4rpx 20rpx rgba(0, 0, 0, .1)">
			<view style="text-align: right; padding-right:5%;">
				<text v-if="amount.discount">
					<text>已优惠</text>
					<text style="color: red;">￥{{ amount.discount }}</text>
				</text>
				<text style="margin-left: 20rpx;">小计</text>
			...
			CheckAddress() {
				this.$request.get('/address/selectAll/', {
					...
					if (this.address.length == 0) {
						let xmOrders = uni.getStorageSync('xm-orders') || {};
						xmOrders.addressId = 0; // 设置 addressId 为 0
						this.addressId = xmOrders.addressId
					} else {
						this.loadAddress()
					}
				})
