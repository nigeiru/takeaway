<template>
	<view style="padding: 20rpx;">
		...
		:class="{ 'category-active' : item.id ===activeId }"
		...
		<view style="margin: 20rpx 0;">
			...
			<view style="display: flex; background-color: #fff; border-radius: 10rpx; padding: 10rpx;">
				...
				<view style="padding: 20rpx; min-height: 70vh" class="goods-container">
					<view style="display: flex; grid-gap: 20rpx; margin-bottom: 20rpx;" v-for="item in goodsList" :key="item.id">
						...
						<view style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
							...
							<view style="font-size: 24rpx;">
							 {{ item.descr!=null ? item.descr : "暂无简介" }}
							 </view>
							<view>
								<text class="mini-btn">
									 {{ item.discount === 1 ? "无折扣" : item.discount*10 + "折" }}
								</text>
							...
							<view >
								...
								<text style="color: orangered; margin-left: 10rpx;">￥{{ item.price*item.discount || 0 }}</text>
...
<style>
	...
	.goods-container {
	  max-height: 70vh; /* 设置最大高度为视口高度的70%（可以根据需要调整） */
	  overflow-y: auto; /* 使内容超出容器时显示垂直滚动条 */
	  background-color: #fff;
	  border-radius: 10rpx;
	  padding: 10rpx;
	}
